...
class SvgCityElement extends SvgMapElement {
    ...
    createDraw(map) {
        ...
        text.setAttribute("x", fastToFixed((map.config.cityIconSize / map.overdrawFactor * 0.5), 0));
        text.setAttribute("y", fastToFixed((map.config.cityIconSize / map.overdrawFactor * map.config.cityLabelDistance * 0.5), 0));
        text.setAttribute("fill", map.config.cityLabelColor);
        text.setAttribute("stroke", map.config.cityLabelStrokeColor);
        text.setAttribute("stroke-width", fastToFixed(map.config.cityLabelStrokeWidth / map.overdrawFactor, 0));
        text.setAttribute("font-size", fastToFixed(map.config.cityLabelFontSize / map.overdrawFactor, 0));
        ...
        if (map.config.cityLabelUseBackground) {
            setTimeout(() => {
                ...
                rect.setAttribute("width", fastToFixed((bbox.width - 4 + map.config.cityLabelBackgroundPaddingRight / map.overdrawFactor + map.config.cityLabelBackgroundPaddingLeft), 0));
                rect.setAttribute("height", fastToFixed(Math.max((bbox.height - 17 + map.config.cityLabelBackgroundPaddingTop / map.overdrawFactor + map.config.cityLabelBackgroundPaddingBottom), 1), 0));
                ...
                rect.setAttribute("stroke-width", fastToFixed(map.config.cityLabelBackgroundStrokeWidth / map.overdrawFactor, 0));
    ...
    updateDraw(map) {
        ...
        if (isFinite(this.x) && isFinite(this.y)) {
            if (Math.abs(this.x - this._lastX) > 0.1 || Math.abs(this.y - this._lastY) > 0.1) {
                ...
                this.svgElement.setAttribute("y", fastToFixed((this.y - map.config.cityIconSize / map.overdrawFactor * 0.5), 2));
                this._lastX = this.x;
                this._lastY = this.y;
            }
        }
