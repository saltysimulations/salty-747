...
exports.LegsProcedure = void 0;
const FixNamingScheme_1 = require("./FixNamingScheme");
const GeoMath_1 = require("./GeoMath");
const HoldDetails_1 = require("./HoldDetails");
...
class LegsProcedure {
    ...
    mapHeadingUntilDistanceFromOrigin(leg, prevLeg) {
        ...
        const course = leg.course + GeoMath_1.GeoMath.getMagvar(prevLeg.infos.coordinates.lat, prevLeg.infos.coordinates.long);
    ...
    mapBearingAndDistanceFromOrigin(leg) {
        const origin = this._facilities.get(leg.originIcao);
        const originIdent = origin.icao.substring(7, 12).trim();
        const course = leg.course + GeoMath_1.GeoMath.getMagvar(origin.lat, origin.lon);
    ...
    mapHeadingToInterceptNextLeg(leg, prevLeg, nextLeg) {
        ...
        if (referenceCoordinates !== undefined && courseToIntercept !== undefined) {
            ...
            return this.buildWaypoint(FixNamingScheme_1.FixNamingScheme.courseToIntercept(course), coordinates);
        }
    }
    /**
    ...
    mapHeadingUntilAltitude(leg, prevLeg) {
        ...
        const course = leg.course + GeoMath_1.GeoMath.getMagvar(prevLeg.infos.coordinates.lat, prevLeg.infos.coordinates.long);
        const coordinates = GeoMath_1.GeoMath.relativeBearingDistanceToCoords(course, distanceInNM, prevLeg.infos.coordinates);
    ...
    mapVectors(leg, prevLeg) {
        const course = leg.course + GeoMath_1.GeoMath.getMagvar(prevLeg.infos.coordinates.lat, prevLeg.infos.coordinates.long);
        const coordinates = GeoMath_1.GeoMath.relativeBearingDistanceToCoords(course, 5, prevLeg.infos.coordinates);
    ...
    mapExactFix(leg, prevLeg) {
        ...
        if (facility) {
            return RawDataMapper_1.RawDataMapper.toWaypoint(facility, this._instrument);
        }
        else {
            const origin = this._facilities.get(leg.originIcao);
            const originIdent = origin.icao.substring(7, 12).trim();
    ...
    mapHold(leg, fixMinusTwo) {
        const facility = this._facilities.get(leg.fixIcao);
        const waypoint = RawDataMapper_1.RawDataMapper.toWaypoint(facility, this._instrument);
        ...
        const holdDetails = HoldDetails_1.HoldDetails.createDefault(leg.course, course);
        holdDetails.turnDirection = leg.turnDirection === 1 ? HoldDetails_1.HoldTurnDirection.Left : HoldDetails_1.HoldTurnDirection.Right;
        holdDetails.entryType = HoldDetails_1.HoldDetails.calculateEntryType(leg.course, course, holdDetails.turnDirection);
...
exports.LegsProcedure = LegsProcedure;
...
//# sourceMappingURL=LegsProcedure.js.map