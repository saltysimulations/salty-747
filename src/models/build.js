/**
 * Copyright (C) 2021 FlyByWire Simulations and its contributors
 * Licensed under GPLv3
 *
 * Source: https://github.com/flybywiresim/a32nx/blob/master/src/model/build.js
 */
...
function combineGltf(pathA, pathB, outputPath) {
    ...
    if (gltfB.materials) {
        for (const material of gltfB.materials) {
            Object.keys(material).forEach((matProperty) => {
    ...
    if (gltfB.meshes) {
        for (const mesh of gltfB.meshes) {
            Object.keys(mesh.primitives[0].attributes).forEach((attribute) => {
            ...
            if (mod.node === gltf.nodes[i].name) {
                ...
                if (samplers.length === mod.outputSamplers.length) {
                    for (let j = 0; j < samplers.length; j++) {
                        ...
                        buffer = replaceAccessorData(buffer, gltf, gltf.accessors[samplers[j].output], mod.outputSamplers[j]);
                    }
                }
            }
        }
...
for (const model of models) {
    for (let i = 0; i < model.gltf.length; i += 1) {
        ...
        if (model.modifications) {
            ...
            let modifiedBin = applyModifications(fs.readFileSync(p(model.bin[i])), p(model.gltf[i]), model.modifications);
            modifiedBin = applyOutputSamplerModifications(modifiedBin, p(model.gltf[i]), model.modifications);
