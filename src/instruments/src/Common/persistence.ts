...
export function usePersistentProperty(propertyName: string, defaultValue?: string): any {
    const [propertyValue, rawPropertySetter] = useState(() => SaltyDataStore.get(propertyName, defaultValue));

    useEffect(() => {
        const unsubscribe = SaltyDataStore.subscribe(propertyName, (key, value) => rawPropertySetter(value));
        return () => {
            unsubscribe();
        };
    }, []);
    ...
    const propertySetter = (value: string) => {
        SaltyDataStore.set(propertyName, value);
