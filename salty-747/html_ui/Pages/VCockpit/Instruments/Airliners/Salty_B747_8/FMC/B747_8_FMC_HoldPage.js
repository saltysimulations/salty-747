 ...
 */
class B747_8_FMC_HoldsPage {
  
  /**
   * Creates an instance of the holds page controller.
   * @param {B747_8_FMC} fmc The instance of the FMC to use with this instance. 
   */
  constructor(fmc) {
    ...
    this._state = {
      ...
      fromWaypointIndex: 0,
      isModifying: false,
      page: B747_8_FMC_HoldsPage.FPLN_HOLD
    };
  }
  ...
  update(forceUpdate = false) {
    ...
    const currentHolds = B747_8_FMC_HoldsPage.getFlightplanHolds(this._fmc);

    if (currentHolds.length === 0) {
      B747_8_FMC_LegsPage.ShowPage1(this._fmc, true);
    }
    else {
      if (this._state.page === B747_8_FMC_HoldsPage.HOLD_LIST) {
        if (currentHolds.length > 1) {
          this.bindHoldListInputs(currentHolds);
          this.renderHoldList(currentHolds);
        }
        else {
          this._state.page = B747_8_FMC_HoldsPage.FPLN_HOLD;
        }
      }
  
      if (this._state.page === B747_8_FMC_HoldsPage.FPLN_HOLD) {
  ...
  bindHoldListInputs(currentHolds) {
    for (let i = 0; i < 6; i++) {
      if (i < 5 && currentHolds[i] !== undefined) {
        this._fmc.onLeftInput[i] = () => B747_8_FMC_HoldsPage.showHoldPage(this._fmc, currentHolds[i].waypoint.ident);
      }
      else if (i >= 5 && currentHolds[i] !== undefined) {
        this._fmc.onRightInput[i - 5] = () => B747_8_FMC_HoldsPage.showHoldPage(this._fmc, currentHolds[i].waypoint.ident);
      }
    }
  ...
  renderFplnHold(currentHold, eta, numPages) {
    ...
    const etaDisplay = `${eta.getHours().toFixed(0).padStart(2, '0')}${eta.getMinutes().toFixed(0).padStart(2, '0')}Z`;
    const holdDetails = currentHold.waypoint.holdDetails;
    let speedSwitch = "ICAO";
    ...
    const rows = [
      ...
      ['\xa0FIX', 'SPD/TGT ALT'],
      [`${currentHold.waypoint.ident}`, this.getMaxKIAS(holdDetails).toFixed(0) + "/-----"],
      ['\xa0QUAD/RADIAL', 'FIX ETA'],
      ['--/---', `${etaDisplay}`],
      ['\xa0INBD CRS/DIR', 'EFC TIME'],
      [`${holdDetails.holdCourse.toFixed(0).padStart(3, '0')}${holdDetails.isHoldCourseTrue ? 'T' : ''}°/${holdDetails.turnDirection === HoldTurnDirection.Left ? 'L' : 'R'} TURN`, efcTime],
      ['\xa0LEG TIME', 'HOLD AVAIL'],
      [`${(holdDetails.legTime / 60).toFixed(1)}MIN`],
      ['\xa0LEG DIST', "BEST SPEED"],
      [`${holdDetails.legDistance.toFixed(1)}NM`, flapsUPmanueverSpeed.toFixed(0)],
      ["__FMCSEPARATOR" ],
      [`${this._state.isModifying ? '<ERASE' : '<NEXT HOLD'}`, `${this.isHoldExiting(currentHold) ? 'CANCEL EXIT>' : this.isHoldActive(currentHold) ? 'EXIT HOLD>' : ''}`]
    ];

    this._fmc.setTemplate(rows);
  }
  ...
  calculateETA(currentHold) {
    ...
    if (activeHoldIndex !== -1 && activeHoldIndex === currentHold.index) {
      const eteSeconds = Math.round(SimVar.GetSimVarValue("L:WT_B747_8_WPT_DISTANCE", "number") / (groundSpeed / 3600));
      ...
      const planePosition = B747_8_FMC_HoldsPage.getPlanePosition();
   ...
   * @param {B747_8_FMC} fmc The FMC to use to look up the holds.
   ...
   * @param {B747_8_FMC} fmc The instance of the FMC to use.
   * @param {string} ident The ident to show. 
   */
  static showHoldPage(fmc, ident) {
    const instance = new B747_8_FMC_HoldsPage(fmc);
    const holds = B747_8_FMC_HoldsPage.getFlightplanHolds(fmc);

    const holdPageIndex = holds.findIndex(x => x.waypoint.ident === ident);
    if (holdPageIndex !== -1) {
      instance._state.pageNumber = holdPageIndex + 1;
      instance._state.page = B747_8_FMC_HoldsPage.FPLN_HOLD;
    }

    instance._state.isModifying = fmc._fpHasChanged;
    B747_8_FMC_HoldsPage.Instance = instance;
  ...
  static showHoldList(fmc) {
    ...
    instance._state.page = B747_8_FMC_HoldsPage.HOLD_LIST;
    instance._state.isModifying = fmc._fpHasChanged;
    B747_8_FMC_HoldsPage.Instance = instance;
  ...
  static handleHoldPressed(fmc) {
    ...
    if (holds.length === 0) {
      B747_8_FMC_LegsPage.ShowPage1(fmc, true);
    }
    else if (holds.length === 1) {
      B747_8_FMC_HoldsPage.showHoldPage(fmc);
    }
    else if (holds.length > 1) {
      B747_8_FMC_HoldsPage.showHoldList(fmc);
    }
  }
}

B747_8_FMC_HoldsPage.Instance = undefined;

B747_8_FMC_HoldsPage.FPLN_HOLD = 'FPLN HOLD';
B747_8_FMC_HoldsPage.HOLD_LIST = 'HOLD LIST';

B747_8_FMC_HoldsPage.NONE = 'NONE';
B747_8_FMC_HoldsPage.ADD = 'ADD';
B747_8_FMC_HoldsPage.CHANGE_EXISTING= 'CHANGE_EXISTING';