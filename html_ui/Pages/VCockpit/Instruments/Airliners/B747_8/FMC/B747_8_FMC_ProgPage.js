    ...
    static ShowPage1(fmc) {
        ...
        if (waypointFrom) {
            ...
            if (isFinite(waypointFrom.fuelWasReached)) {
                waypointFromFuelCell = waypointFrom.getFuelWasReached(units).toFixed(0);
            }
        }
        let speed = Simplane.getGroundSpeed();
        let currentTime = SimVar.GetGlobalVarValue("LOCAL TIME", "seconds");
        if (!units) {
            let currentFuel = SimVar.GetSimVarValue("FUEL TOTAL QUANTITY", "liters") * SimVar.GetSimVarValue("FUEL WEIGHT PER GALLON", "kilograms") / 1000;
        } else {
