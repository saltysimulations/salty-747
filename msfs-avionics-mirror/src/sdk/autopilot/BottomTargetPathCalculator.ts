import { BitFlags, ReadonlySubEvent, SubEvent, UnitType } from '..';
import { EventBus } from '../data/EventBus';
import { FlightPlan, LegDefinitionFlags, FlightPlanner, FlightPlannerEvents, FlightPlanLegEvent, FlightPlanSegmentEvent, FlightPlanSegmentType, LegDefinition, FlightPlanCalculatedEvent, FlightPlanLegIterator } from '../flightplan';
import { AltitudeRestrictionType, LegType } from '../navigation';
import { VNavPathCalculator } from './VNavPathCalculator';
import { VNavConstraint, VNavLeg, VerticalFlightPhase, VerticalFlightPlan } from './VerticalNavigation';
import { VNavUtils } from './VNavUtils';
import { ADCEvents } from '../instruments';
import { VNavControlEvents } from './VNavControlEvents';
...
export class BottomTargetPathCalculator implements VNavPathCalculator {
  ...
  constructor(private readonly bus: EventBus,
    ...
    bus.getSubscriber<ADCEvents>().on('alt').whenChangedBy(1).handle(alt => this.currentAltitude = UnitType.FOOT.convertTo(alt, UnitType.METER));
